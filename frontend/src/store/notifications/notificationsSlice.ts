...
export interface NotificationItem { id: string; title: string; body: string; isRead: boolean; resourceType: string | null; resourceId: string | null; createdAt: string; }
interface NotificationsState { items: NotificationItem[]; unreadCount: number; loading: boolean; }
...
const notificationsSlice = createSlice({
  ...
  reducers: {
    ...
    markItemRead: (state, action: PayloadAction<string>) => { const item = state.items.find(n => n.id === action.payload); if (item) item.isRead = true; if (state.unreadCount > 0) state.unreadCount -= 1; },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(markReadThunk.fulfilled, (state, action) => { const item = state.items.find(n => n.id === action.payload); if (item && !item.isRead) { item.isRead = true; if (state.unreadCount > 0) state.unreadCount -= 1; } })
