...
vi.mock('@mui/x-data-grid', () => ({
  DataGrid: ({ rows, loading, columns }: { rows: LeaveRequestDto[]; loading: boolean; columns: { field: string; renderCell?: (p: { row: LeaveRequestDto; value: unknown }) => React.ReactNode }[] }) => {
    ...
    return (
      <div data-testid="approvals-grid">
        {rows.map(row => { const actionsCol = columns.find(c => c.field === 'actions'); return (<div key={row.id} data-testid={`grid-row-${row.id}`}><span>{row.employeeName}</span><span>{row.leaveTypeName}</span>{actionsCol?.renderCell?.({ row, value: undefined })}</div>); })}
      </div>
    );
  },
}));

vi.mock('../api/approvalsApi', () => ({ getPendingApprovals: vi.fn(), approveRequest: vi.fn(), rejectRequest: vi.fn() }));
...
function makeRow(overrides: Partial<LeaveRequestDto> = {}): LeaveRequestDto {
  return { id: 'lr-001', employeeName: 'Alice Smith', leaveTypeName: 'Annual Leave', startDate: '2026-08-10', endDate: '2026-08-12', computedDays: 3, isRetroactive: false, status: 'Pending', documentUrl: null, reason: 'Vacation', ...overrides };
}
...
function makeStore() {
  return configureStore({
    ...
    preloadedState: { auth: { user: { id: 'mgr-1', email: 'mgr@example.com', fullName: 'Manager User', role: 'Manager' }, accessToken: 'tok', refreshToken: 'ref', isAuthenticated: true, isLoading: false, error: null } },
    middleware: (gm) => gm({ thunk: true }),
  });
}

async function renderPage() {
  if (!ApprovalsPage) ApprovalsPage = (await import('../pages/approvals/ApprovalsPage')).default;
  return render(<Provider store={makeStore()}><BrowserRouter><ApprovalsPage /></BrowserRouter></Provider>);
}
...
describe('UT-FE-35: ApprovalsPage shows loading indicator initially', () => {
  it('shows progressbar while fetching', async () => {
    vi.mocked(approvalsApi.getPendingApprovals).mockReturnValue(new Promise(() => {}));
    await renderPage();
...
describe('UT-FE-36: ApprovalsPage shows empty grid when no pending approvals', () => {
  it('shows empty grid state when API returns empty list', async () => {
    ...
    await renderPage();
    await waitFor(() => { expect(screen.getByTestId('empty-grid')).toBeInTheDocument(); });
  });
});
...
describe('UT-FE-37: ApprovalsPage shows error alert on fetch failure', () => {
  it('shows fetch-error alert when getPendingApprovals rejects', async () => {
    vi.mocked(approvalsApi.getPendingApprovals).mockRejectedValueOnce(new Error('Network'));
    await renderPage();
    await waitFor(() => { expect(screen.getByTestId('fetch-error')).toBeInTheDocument(); expect(screen.getByText('Failed to load pending approvals. Please try again.')).toBeInTheDocument(); });
  });
});
...
describe('UT-FE-38: ApprovalsPage renders rows with action buttons', () => {
  it('shows employee name, leave type, and action buttons', async () => {
    vi.mocked(approvalsApi.getPendingApprovals).mockResolvedValue({ items: [makeRow()], total: 1, page: 1, limit: 10 });
    await renderPage();
    await waitFor(() => { expect(screen.getByTestId('grid-row-lr-001')).toBeInTheDocument(); });
...
describe('UT-FE-39: Approve removes row optimistically', () => {
  it('removes the row immediately when approve is clicked and API succeeds', async () => {
    vi.mocked(approvalsApi.getPendingApprovals).mockResolvedValue({ items: [makeRow({ id: 'lr-001' }), makeRow({ id: 'lr-002', employeeName: 'Bob' })], total: 2, page: 1, limit: 10 });
    vi.mocked(approvalsApi.approveRequest).mockResolvedValueOnce(undefined);
    await renderPage();
    await waitFor(() => { expect(screen.getByTestId('approve-btn-lr-001')).toBeInTheDocument(); });
    fireEvent.click(screen.getByTestId('approve-btn-lr-001'));
    await waitFor(() => { expect(screen.queryByTestId('grid-row-lr-001')).not.toBeInTheDocument(); });
...
describe('UT-FE-41: Reject dialog requires a comment before submitting', () => {
  it('opens reject dialog when Reject button is clicked', async () => {
    vi.mocked(approvalsApi.getPendingApprovals).mockResolvedValue({ items: [makeRow()], total: 1, page: 1, limit: 10 });
    await renderPage();
    await waitFor(() => { expect(screen.getByTestId('reject-btn-lr-001')).toBeInTheDocument(); });
  ...
  it('reject submit button is disabled when comment is empty', async () => {
    ...
    await waitFor(() => { expect(screen.getByTestId('reject-btn-lr-001')).toBeInTheDocument(); });
    fireEvent.click(screen.getByTestId('reject-btn-lr-001'));
    await waitFor(() => { expect(screen.getByTestId('reject-submit-btn')).toBeDisabled(); });
  });
  ...
  it('enables submit when comment is filled and calls rejectRequest', async () => {
    ...
    fireEvent.click(screen.getByTestId('reject-btn-lr-001'));
    await waitFor(() => { expect(screen.getByTestId('reject-dialog')).toBeInTheDocument(); });
    fireEvent.change(screen.getByTestId('reject-comment-input'), { target: { value: 'Not enough leave balance.' } });
    await waitFor(() => { expect(screen.getByTestId('reject-submit-btn')).not.toBeDisabled(); });
    fireEvent.click(screen.getByTestId('reject-submit-btn'));
    await waitFor(() => { expect(approvalsApi.rejectRequest).toHaveBeenCalledWith('lr-001', 'Not enough leave balance.'); });
  });
});
