...
export default function LeaveTypesPage() {
  ...
  const { control, handleSubmit, reset, watch, setValue } = useForm<FormValues>({
    defaultValues: { name: '', accrualType: 0, maxDaysPerYear: '', requiresDocument: false },
  });

  const accrualTypeValue = watch('accrualType');

  useEffect(() => {
    if (accrualTypeValue === 2) { setValue('maxDaysPerYear', ''); }
  ...
  function openEditDialog(row: LeaveTypeDto) {
    setEditing(row);
    reset({
      name: row.name, accrualType: row.accrualType,
  ...
  async function onSubmit(values: FormValues) {
    const payload: CreateLeaveTypePayload = {
      ...
      maxDaysPerYear: values.accrualType === 2 || values.maxDaysPerYear === '' ? null : Number(values.maxDaysPerYear),
  ...
  async function handleDelete(id: string) {
    try {
      ...
      if (!deleted) { showToast('Cannot delete — leave balances exist', 'error'); return; }
  ...
  const columns: GridColDef<LeaveTypeDto>[] = [
    ...
    {
      field: 'maxDaysPerYear', headerName: 'Max Days/Year', flex: 1, minWidth: 130,
      renderCell: (params: GridRenderCellParams<LeaveTypeDto>) =>
        params.value != null ? String(params.value) : <em style={{ color: '#888' }}>Unlimited</em>,
    },
    {
      field: 'accrualType', headerName: 'Accrual Type', flex: 1, minWidth: 120,
      renderCell: (params: GridRenderCellParams<LeaveTypeDto>) => (
        <Chip label={ACCRUAL_TYPE_LABELS[params.value as number] ?? 'Unknown'} size="small" variant="outlined" />
      ),
    },
    {
      field: 'requiresDocument', headerName: 'Doc Required', flex: 0.8, minWidth: 120,
      renderCell: (params: GridRenderCellParams<LeaveTypeDto>) =>
        params.value ? <Chip label="Yes" color="warning" size="small" /> : <Chip label="No" size="small" />,
    },
    {
      field: 'isActive', headerName: 'Status', flex: 0.8, minWidth: 100,
      renderCell: (params: GridRenderCellParams<LeaveTypeDto>) =>
        params.value
    ...
    {
      field: 'actions', headerName: 'Actions', sortable: false, flex: 0.8, minWidth: 100,
      renderCell: (params: GridRenderCellParams<LeaveTypeDto>) => (
        <Box>
          <Tooltip title="Edit">
            <IconButton size="small" onClick={() => openEditDialog(params.row)}><EditIcon fontSize="small" /></IconButton>
          </Tooltip>
          <Tooltip title="Delete">
            <IconButton size="small" color="error" onClick={() => void handleDelete(params.row.id)}><DeleteIcon fontSize="small" /></IconButton>
          </Tooltip>
        </Box>
      ),
    },
    ...
    <Box p={3}>
      ...
      {loading ? (
        ...
        <DataGrid
          ...
          getRowClassName={(params) => !params.row.isActive ? 'row--inactive' : ''}
          sx={{ '& .row--inactive': { opacity: 0.55 } }}
        />
      )}
          ...
          <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            <Controller name="name" control={control} rules={{ required: 'Name is required' }}
              render={({ field, fieldState }) => (
                <TextField {...field} label="Name" required fullWidth error={!!fieldState.error} helperText={fieldState.error?.message} />
              )} />
            <Controller name="accrualType" control={control}
              render={({ field }) => (
                <TextField {...field} select label="Accrual Type" fullWidth SelectProps={{ native: false }}>
                  {ACCRUAL_OPTIONS.map((opt) => <MenuItem key={opt.value} value={opt.value}>{opt.label}</MenuItem>)}
                </TextField>
              )} />
            <Controller name="maxDaysPerYear" control={control}
              render={({ field }) => (
                <TextField {...field} label="Max Days Per Year" type="number" fullWidth
                  disabled={accrualTypeValue === 2} inputProps={{ min: 0, step: 0.5 }}
                  helperText={accrualTypeValue === 2 ? 'Not applicable for Unlimited accrual' : 'Leave blank for unlimited'} />
              )} />
            <Controller name="requiresDocument" control={control}
              render={({ field }) => (
                <FormControlLabel
