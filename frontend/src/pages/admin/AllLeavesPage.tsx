...
export default function AllLeavesPage() {
  ...
  const handleRevoke = async (id: string) => {
    ...
    try {
      ...
      setSnackbar({ message: 'Failed to revoke request. Please try again.', severity: 'error' });
    } finally {
      setRevokingId(null);
    }
  };
  ...
  return (
    <Box p={4}>
      <Box display="flex" alignItems="center" justifyContent="space-between" mb={3}>
        ...
        <Button variant="outlined" onClick={() => { void handleExportCsv(); }} data-testid="export-csv-button">
          Export CSV
        </Button>
      </Box>
                ...
                <TableRow key={row.id} data-testid={`leave-row-${row.id}`}>
                  ...
                  <TableCell align="center">
                    {row.status === 'Approved' && (
                      ...
                      >
                        {revokingId === row.id ? <CircularProgress size={14} color="inherit" /> : 'Revoke'}
                      </Button>
                    )}
                  </TableCell>
                </TableRow>
