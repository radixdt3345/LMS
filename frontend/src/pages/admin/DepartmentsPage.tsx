...
function DepartmentsPage(): JSX.Element {
  ...
  const handleDialogClose = () => { if (!isSubmitting) setDialogOpen(false) }

  const onSubmit: SubmitHandler<DeptFormValues> = async (values) => {
    try {
      if (editTarget !== null) {
        await departmentsApi.update(editTarget.id, { name: values.name, description: values.description || undefined })
        showSnackbar('Department updated.', 'success')
      } else {
        await departmentsApi.create({ name: values.name, description: values.description || undefined })
  ...
  const handleDeleteClick = (dept: Department) => { setDeleteTarget(dept); setDeleteDialogOpen(true) }
  ...
  const handleDeleteCancel = () => { if (!deleteLoading) { setDeleteDialogOpen(false); setDeleteTarget(null) } }

  const columns: GridColDef[] = [
    { field: 'name', headerName: 'Name', flex: 1, minWidth: 160 },
    { field: 'description', headerName: 'Description', flex: 2, minWidth: 200,
      valueGetter: (params: GridRenderCellParams) => (params.value as string | null) ?? '—' },
    { field: 'employeeCount', headerName: 'Employees', width: 110, type: 'number' },
    ...
    { field: 'actions', headerName: 'Actions', width: 120, sortable: false,
      renderCell: (params: GridRenderCellParams) => {
        ...
        return (
          <Box>
            <IconButton size="small" aria-label="edit" onClick={() => { handleEditClick(row) }}><EditIcon fontSize="small" /></IconButton>
            <IconButton size="small" color="error" aria-label="delete" onClick={() => { handleDeleteClick(row) }}><DeleteIcon fontSize="small" /></IconButton>
          </Box>
        )
      } },
  ]
  ...
  return (
    <Box p={3}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddClick}>Add Department</Button>
      </Box>
      ...
      <Box sx={{ height: 520 }}>
        <DataGrid
          ...
          loading={loading}
          getRowClassName={(params) => (params.row as Department).isActive ? '' : 'row-inactive'}
          sx={{ '& .row-inactive': { opacity: 0.55, backgroundColor: 'action.hover' }, '& .MuiDataGrid-cell:focus': { outline: 'none' } }}
        />
      </Box>

      <Dialog open={dialogOpen} onClose={handleDialogClose} maxWidth="sm" fullWidth>
        <form onSubmit={(e) => { void handleSubmit(onSubmit)(e) }}>
          <DialogTitle>{editTarget !== null ? 'Edit Department' : 'Add Department'}</DialogTitle>
          <DialogContent>
            <TextField label="Name" fullWidth margin="normal"
              {...register('name', { required: 'Department name is required.' })}
              error={errors.name !== undefined} helperText={errors.name?.message} autoFocus />
      ...
      <Dialog open={deleteDialogOpen} onClose={handleDeleteCancel} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <DialogContentText>
            Deactivate <strong>{deleteTarget?.name ?? ''}</strong>? This will hide it from active department lists.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleDeleteCancel} disabled={deleteLoading}>Cancel</Button>
          <Button color="error" variant="contained" disabled={deleteLoading} onClick={() => { void handleDeleteConfirm() }}>
