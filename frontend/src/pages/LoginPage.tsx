...
export default function LoginPage() {
  ...
  const onSubmit: SubmitHandler<LoginFormValues> = data => { dispatch(loginRequest({ email: data.email, password: data.password })); };
  const handleMsalLogin = () => { instance.loginRedirect(msalLoginRequest).catch((err: unknown) => console.error('[MSAL] loginRedirect error', err)); };
  ...
  return (
    <Box display="flex" alignItems="center" justifyContent="center" minHeight="100vh" bgcolor="grey.100">
      <Card sx={{ width: '100%', maxWidth: 420, p: 2 }} elevation={3}>
        <CardContent>
          <Typography variant="h5" fontWeight={600} textAlign="center" mb={3}>Leave Management System</Typography>
          {error && <Alert severity="error" sx={{ mb: 2 }} role="alert">{error}</Alert>}
          <Box component="form" onSubmit={handleSubmit(onSubmit)} noValidate>
            <TextField label="Email address" type="email" fullWidth margin="normal" autoComplete="email"
            ...
            <Button type="submit" variant="contained" fullWidth size="large" disabled={isLoading} sx={{ mt: 2, mb: 1 }}>
              {isLoading ? <CircularProgress size={22} color="inherit" /> : 'Sign In'}
            </Button>
          </Box>
          <Divider sx={{ my: 2 }}>or</Divider>
          <Button variant="outlined" fullWidth size="large" onClick={handleMsalLogin} disabled={isLoading}>Sign in with Microsoft</Button>
        </CardContent>
      </Card>
    </Box>
  );
