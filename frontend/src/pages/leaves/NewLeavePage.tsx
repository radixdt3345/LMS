...
export default function NewLeavePage() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => { fetchPreview(startDate, endDate, leaveTypeId); }, 500);
  ...
  function validate(): boolean {
    ...
    if (startDate && endDate && startDate > endDate) next.endDate = 'End date must be on or after start date.';
    if (!leaveTypeId) next.leaveTypeId = 'Please select a leave type.';
    if (!reason.trim()) next.reason = 'Reason is required.';
    if (requiresDocument && !documentUrl.trim()) next.documentUrl = 'Document URL is required for this leave type.';
    setErrors(next);
  ...
  return (
    <Box p={4} maxWidth={600} mx="auto">
      <Typography variant="h5" fontWeight={600} mb={3}>New Leave Request</Typography>
      {leaveTypesError && <Alert severity="error" sx={{ mb: 2 }}>{leaveTypesError}</Alert>}
