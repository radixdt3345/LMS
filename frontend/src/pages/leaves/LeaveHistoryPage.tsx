...
import {
  Alert, Box, Button, Chip, CircularProgress, Paper, Snackbar,
  Table, TableBody, TableCell, TableContainer, TableFooter, TableHead,
  TablePagination, TableRow, Typography, type ChipProps,
} from '@mui/material';
        ...
        <TableContainer component={Paper} elevation={2}>
          <Table aria-label="leave history table" data-testid="leave-history-table">
              ...
              <TableRow key={row.id} data-testid={`leave-row-${row.id}`}>
                ...
                <TableCell align="center">
                  {(row.status === 'Pending' || row.status === 'Draft') && (
                    <Button variant="outlined" size="small" color="error" disabled={cancellingId === row.id}
                      onClick={() => void handleCancel(row.id)} data-testid={`cancel-btn-${row.id}`}>
                      {cancellingId === row.id ? <CircularProgress size={14} color="inherit" /> : 'Cancel'}
                    </Button>
                  )}
                </TableCell>
              </TableRow>
            ...
            <TableFooter><TableRow>
              <TablePagination count={total} page={page} rowsPerPage={PAGE_LIMIT} rowsPerPageOptions={[PAGE_LIMIT]}
                onPageChange={(_event, newPage) => setPage(newPage)} data-testid="pagination" />
