...
public class CompOffController : ControllerBase
{
    ...
    /// <summary>
    /// GET /api/v1/comp-off/requests/pending
    /// Returns comp-off requests awaiting approval, each with the employee's
    /// name. A Manager sees only their direct reports; HR Admin and Super Admin
    /// see all. Without this an approver has no way to find requests to act on.
    /// </summary>
    ...
    public async Task<IActionResult> GetPendingRequests()
    {
        ...
        var isHrAdmin = User.IsInRole("HRAdmin") || User.IsInRole("SuperAdmin");

        var result = await _requestService.GetPendingForApproverAsync(userId.Value, isHrAdmin);
        if (!result.IsSuccess)
            return StatusCode(result.StatusCode,
                new { success = false, error = new { message = result.Error } });
