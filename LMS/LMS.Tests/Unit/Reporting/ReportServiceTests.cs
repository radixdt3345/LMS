...
public class ReportServiceTests
{
    ...
    {
        var opts = new DbContextOptionsBuilder<LmsDbContext>().UseInMemoryDatabase(name).Options;
    ...
    public async Task UT57_GetUtilizationAsync_ThreeApprovedRequestsSameDept_ReturnsCorrectAggregation()
    {
        ...
        var deptId = Guid.NewGuid(); var leaveTypeId = Guid.NewGuid();
        var emp1 = Guid.NewGuid(); var emp2 = Guid.NewGuid(); var emp3 = Guid.NewGuid();
        db.Departments.Add(new Department { Id = deptId, Name = "Engineering", IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        db.LeaveTypes.Add(new LeaveType { Id = leaveTypeId, Name = "Annual", IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        db.Users.AddRange(
    ...
    public async Task UT58_GetTrendsAsync_ApprovedAndRejectedAcrossMonths_ReturnsCorrectCounts()
    {
        ...
        var leaveTypeId = Guid.NewGuid(); var emp = Guid.NewGuid();
        db.LeaveTypes.Add(new LeaveType { Id = leaveTypeId, Name = "Annual", IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        db.Users.Add(new User { Id = emp, Email = "e@t.com", IsActive = true, Role = UserRole.Employee, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        db.LeaveRequests.AddRange(
    ...
    public async Task UT59_GetComplianceAsync_FiveEmployeesThreeWithRequests_Returns60Percent()
    {
        ...
        db.Users.AddRange(empIds.Select(id => new User { Id = id, Email = $"{id}@t.com", IsActive = true, Role = UserRole.Employee, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow }));
        db.LeaveRequests.AddRange(
    ...
    public async Task UT60_GetEmployeeDashboardAsync_ReturnsBalancesAndRecentRequests()
    {
        ...
        var employeeId = Guid.NewGuid(); var annualTypeId = Guid.NewGuid(); var sickTypeId = Guid.NewGuid();
        ...
        db.Users.Add(new User { Id = employeeId, Email = "emp@t.com", IsActive = true, Role = UserRole.Employee, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        ...
        Assert.Equal(20m, annual.Allocated); Assert.Equal(5m, annual.Used); Assert.Equal(15m, annual.Available);
    ...
    public async Task UT61_GetManagerDashboardAsync_ReturnsPendingRequestsAndTeamSize()
    {
        ...
        var managerId = Guid.NewGuid(); var emp1Id = Guid.NewGuid(); var emp2Id = Guid.NewGuid(); var leaveTypeId = Guid.NewGuid();
        db.LeaveTypes.Add(new LeaveType { Id = leaveTypeId, Name = "Annual", IsActive = true, CreatedAt = DateTime.UtcNow, UpdatedAt = DateTime.UtcNow });
        db.Users.AddRange(
